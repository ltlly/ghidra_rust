//! VTSessionDB -- database-backed VTSession implementation.
//!
//! Corresponds to Ghidra's `VTSessionDB` Java class.
...
use crate::versiontracking::vt_match_set::VtMatchSetImpl;
use crate::versiontracking::vt_session::VtSessionTrait;

/// Database version constant for schema compatibility.
pub const VT_DB_VERSION: i32 = 3;

/// Database version tracking session.
///
/// Wraps a SQLite connection and provides persistence for match sets,
/// associations, tags, and address correlators.
///
/// This is the Rust equivalent of Ghidra's `VTSessionDB` Java class.
...
impl VtSessionDB {
    ...
    fn initialize_tables(&self) -> VtResult<()> {
        ...
        conn.execute(
            ...
            params!["DB_VERSION", VT_DB_VERSION.to_string()],
        ).map_err(VtError::DatabaseError)?;
...
impl VtSessionTrait for VtSessionDB {
    ...
    fn create_match_set(&mut self, correlator_name: &str) -> u64 {
        match self.create_match_set(correlator_name) {
            Ok(id) => id as u64,
            Err(_) => 0,
        }
    }
    ...
    fn match_sets(&self) -> Vec<&VtMatchSetImpl> {
        // VtSessionDB uses VtMatchSetDB, not VtMatchSetImpl
        // Return empty vec as they are different types
        Vec::new()
    }
    ...
    fn create_match_tag(&mut self, name: &str) -> VtMatchTag {
        match self.create_match_tag(name) {
            Ok(tag) => tag,
            Err(_) => VtMatchTag::new(name),
        }
    }

    fn delete_match_tag(&mut self, tag: &VtMatchTag) {
        let _ = self.delete_match_tag(tag);
    }

    fn match_tags(&self) -> &[VtMatchTag] {
        // VtSessionDB uses VtMatchTagDB internally
        &[]
    }
    ...
    fn get_matches_for_association(&self, _association: &VtAssociation) -> Vec<&VtMatchImpl> {
        Vec::new()
    }
...
mod tests {
    ...
    fn test_session_db_db_version() {
        ...
        assert_eq!(version, VT_DB_VERSION);
    }
