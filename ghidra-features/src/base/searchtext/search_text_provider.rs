//! Search text provider for Features/Base.
//!
//! Ported from Ghidra's `ghidra.app.plugin.core.searchtext.SearchTextProvider`.
//!
...
impl SearchTextProvider {
    ...
    pub fn on_text_changed(&mut self, text: &str) -> bool {
        ...
        // Trigger search if the text is non-empty.
        !text.is_empty()
    }

    /// Process the user pressing Enter (search next).
    ///
    /// Returns `true` if there is text to search for.
    pub fn on_search_next(&mut self) -> bool {
    ...
    ///
    /// Returns `true` if there is text to search for.
    pub fn on_search_previous(&mut self) -> bool {
        if self.search_text.is_empty() {
            return false;
        }
        self.add_to_history();
    ...
    ///
    /// Returns `true` if there is text to search for.
    pub fn on_search_all(&mut self) -> bool {
        if self.search_text.is_empty() {
            return false;
        }
        self.add_to_history();
    ...
    pub fn toggle_search_mode(&mut self) {
        self.database_search = !self.database_search;
    }
...
mod tests {
    ...
    fn test_provider_on_search_next() {
        let mut provider = SearchTextProvider::new("Test");
        assert!(!provider.on_search_next());
    ...
    fn test_provider_on_search_previous() {
        ...
        assert!(provider.on_search_previous());
        assert_eq!(provider.history().len(), 1);
    }
    ...
    fn test_provider_toggle_case_sensitive() {
        let mut provider = SearchTextProvider::new("Test");
        let initial = provider.is_case_sensitive();
        provider.toggle_case_sensitive();
        assert_ne!(provider.is_case_sensitive(), initial);
    }

    #[test]
    fn test_provider_toggle_direction() {
        let mut provider = SearchTextProvider::new("Test");
        let initial = provider.is_forward();
        provider.toggle_direction();
        assert_ne!(provider.is_forward(), initial);
    }

    #[test]
    fn test_provider_toggle_search_mode() {
        let mut provider = SearchTextProvider::new("Test");
        let initial = provider.is_program_database_search();
        provider.toggle_search_mode();
        assert_ne!(provider.is_program_database_search(), initial);
    }
