//! Overview provider -- listing margin bar with actions, navigation, and refresh.
//!
//! Ported from Ghidra's `OverviewColorComponent` Java class (which implements
//! `ListingOverviewProvider`).
//!
//! This module provides the provider implementation that covers:
...
//! # Architecture
//!
//! - [`OverviewProvider`] -- full-featured overview provider
...
use crate::overview::{OverviewColorComponent, OverviewColorService, RgbColor};
...
/// use ghidra_core::Address;
///
/// let mut provider = OverviewProvider::new("OverviewBar");
...
impl OverviewProvider {
    ...
    pub fn new(title: impl Into<String>) -> Self {
        Self {
            title: title.into(),
            visible: false,
            component: OverviewColorComponent::new(Box::new(crate::overview::StubColorService::new(
                "default".to_string(),
            ))),
...
impl std::fmt::Debug for OverviewProvider {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("OverviewProvider")
...
impl Default for OverviewProvider {
    fn default() -> Self {
...
mod tests {
    ...
    // -- OverviewProvider tests --

    #[test]
    fn test_provider_new() {
        let provider = OverviewProvider::new("TestBar");
    ...
    fn test_provider_visibility() {
        let mut provider = OverviewProvider::new("TestBar");
        assert!(!provider.is_visible());
    ...
    fn test_provider_with_service() {
        ...
        let mut provider = OverviewProvider::with_service("OverviewBar", service);
        provider.set_height(100);
