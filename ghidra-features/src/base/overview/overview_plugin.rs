//! Overview color plugin -- manages overview color services.
//!
//! Ported from Ghidra's `ghidra.app.plugin.core.overview.OverviewColorPlugin`.
//!
//! This module provides the plugin that discovers, registers, and manages
//! [`OverviewColorService`] instances.  It creates toggle actions for each
//! service and installs/removes [`OverviewProvider`]s as indicated by user
//! actions.
//!
//! # Architecture
//!
//! - [`OverviewPlugin`] -- top-level plugin with full lifecycle management
...
//! - [`OverviewAction`] -- install/uninstall intent from a toggle

use std::collections::HashMap;

use crate::overview::{OverviewColorComponent, OverviewColorService, OverviewSaveState, RgbColor};
...
/// # Lifecycle
///
/// 1. [`new()`](OverviewPlugin::new) -- create plugin
/// 2. [`init()`](OverviewPlugin::init) -- discover services, create actions
/// 3. [`program_activated()`](OverviewPlugin::program_activated) -- notify services
/// 4. [`program_deactivated()`](OverviewPlugin::program_deactivated) -- clear services
/// 5. [`cleanup()`](OverviewPlugin::cleanup) -- uninstall all, dispose
///
/// # Example
///
/// ```
/// use ghidra_features::base::overview::*;
/// use ghidra_features::overview::*;
///
/// let mut plugin = OverviewPlugin::new("OverviewColorPlugin");
/// plugin.init();
...
impl std::fmt::Debug for OverviewPlugin {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("OverviewPlugin")
...
impl OverviewPlugin {
    ...
    pub fn install_overview(&mut self, service_index: usize) {
        ...
        let component = OverviewColorComponent::new(Box::new(crate::overview::StubColorService::new(svc_name)));
...
impl Default for OverviewPlugin {
    fn default() -> Self {
...
mod tests {
    ...
    #[test]
    fn test_plugin_new() {
        let plugin = OverviewPlugin::new("TestPlugin");
    ...
    #[test]
    fn test_plugin_init() {
        let mut plugin = OverviewPlugin::new("TestPlugin");
        plugin.init();
        assert!(plugin.is_initialized());
    }
    ...
    fn test_plugin_config_save_restore() {
        ...
        let mut plugin2 = OverviewPlugin::new("TestPlugin");
        plugin2.init();
